...
# NOTE: Monkey patched version of original
# Can be removed when fetchOHLCV added
# https://github.com/ccxt/ccxt/issues/2815
# Changes:
# added fetchOHLCV to has
# add fetch_ohlcv and parse_ohlcv function
# importing requests
import requests
...
class cryptopia (Exchange):
    ...
    def describe(self):
        return self.deep_extend(super(cryptopia, self).describe(), {
            ...
            # TODO: 1m and 5m intervals faked
            # bot should know this lower limit instead of faking
            'timeframes': {
                '1m': 1,
                '5m': 5,
                '15m': 15,
    ...
    def parse_ohlcv(self, ohlcv, market=None, timeframe='15m', since=None, limit=None):
        return [
            ohlcv[0],
            float(ohlcv[1]),
            float(ohlcv[2]),
    ...
    # https://www.cryptopia.co.nz/Exchange/GetTradePairChart?tradePairId=4369&dataRange=4&dataGroup=240
    #
    # dataRange
    #
    # 0 = 1d
    ...
    # 1d = 1440
    # 1w = 10080
    # TODO: since not implemented, just a generic 30 day when none
    # NOTE: For some reason dataRange=0 returns the last month?
    def fetch_ohlcv(self, symbol, timeframe='5m', since=None, limit=None, params={}):
        ...
        if since is not None:
            ...
            params['dataRange'] = 0
        response = requests.get('https://www.cryptopia.co.nz/Exchange/GetTradePairChart', params=params)
        candles = response.json()['Candle']
        volumes = response.json()['Volume']
        count = 0
        for candle in candles :
            candle.append(volumes[count]['y'])
            count = count + 1
